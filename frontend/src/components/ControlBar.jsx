...
export default function ControlBar({
  step,
  returnName, setReturnName,
  returnInfo, tables, tableName, setTableName,
  dateStr, setDateStr, dateHint,
  periods, setPeriods,
  loading, error,
  candidates,
  handleFindReturn, handleCompute, handleReset, handleSelectCandidate,
  onVisualize, vizOpen,
}) {
  ...
  return (
    <div className="ctrl-bar">
      ...
      <div className="ctrl-row">
        ...
        >
          {searchBusy ? <><span className="spinner" />&thinsp;Searching&hellip;</> : 'Search'}
        </button>
          ...
          <>
            ...
            <div className="ctrl-return-tag">
              ...
              <span className="vt-badge vt-badge-prev">
                {(returnInfo.tables || []).length}&thinsp;table{(returnInfo.tables || []).length !== 1 ? 's' : ''}
              </span>
            </div>
          </>
        )}

        {error && <span className="ctrl-error-inline">&#x26A0;&thinsp;{error}</span>}
      </div>

      {/* Disambiguation pick-list */}
      {step === VARIANCE_STEPS.DISAMBIGUATE && candidates != null && candidates.length > 0 && (
        <div className="ctrl-row ctrl-disambig-row">
          <span className="ctrl-label ctrl-disambig-label">
            ...
            &thinsp;<strong>&ldquo;{returnName}&rdquo;</strong> &mdash; pick one:
          </span>
          <div className="ctrl-disambig-list">
            {candidates.map((c) => {
              ...
              return (
                <button
                  key={c.return_id}
                  className={'ctrl-disambig-item' + (!c.has_mapping ? ' ctrl-disambig-no-map' : '')}
                  onClick={() => c.has_mapping && handleSelectCandidate(c)}
                  disabled={!c.has_mapping}
                  title={!c.has_mapping ? 'No table-mapping data for this return' : undefined}
                >
                  <span className={'ctrl-score-badge ' + badge.cls}>{badge.label}</span>
                  <span className="ctrl-disambig-name">{c.return_name}</span>
                  <span className="ctrl-disambig-meta">{freqLabel(c.report_freq) || '—'}</span>
                  {!c.has_mapping && (
                    <span className="ctrl-disambig-nomapping">no mapping</span>
                  )}
                </button>
              )
            })}
      ...
      {/* Row 2: Query config — only visible once a return is found */}
      {returnInfo && (
        <div className="ctrl-row">
          ...
          <input
            ...
            title={'Format: DD-MMM-YYYY' + (dateHint.hint ? ' · ' + dateHint.hint : '')}
          />
          ...
          <div className="ctrl-chips">
            {[1, 2, 3].map(n => (
              <button
                key={n}
                className={'ctrl-chip' + (periods === n ? ' ctrl-chip-on' : '')}
          ...
          <div className="ctrl-actions">
            {!isResult ? (
              <button className="btn btn-sm" disabled={!canCompute} onClick={handleCompute}>
                {loading
              ...
              <>
                <button className="btn btn-sm btn-secondary" onClick={handleReset} title="Start a new query">
                  New Query
                </button>
                <button
                ...
                >
                  {loading ? <><span className="spinner" />&thinsp;Recomputing&hellip;</> : 'Recompute'}
                </button>
                <button
                  className={'btn btn-sm' + (vizOpen ? ' ctrl-viz-btn-on' : ' ctrl-viz-btn')}
                  onClick={onVisualize}
                  title={vizOpen ? 'Collapse visualization panel' : 'Open visualization panel'}
                >
                  {vizOpen ? 'Hide Viz' : 'Visualize Data'}
                </button>
              </>
            )}
          </div>
